...
When streaming games, you often need to balance between visual quality (resolution) and smooth motion (FPS). Moonbeam allows you to prioritize one over the other using the priority setting, helping you achieve the best possible experience based on your preferences and network conditions.

## Setting Priority

Use the `--priority` option to set your streaming priority:
...
Moonbeam supports two priority options:

1. `fps` (default): Prioritizes maintaining the highest possible frame rate, potentially at the cost of resolution.
2. `resolution`: Prioritizes maintaining the highest possible resolution, potentially at the cost of frame rate.

:::note
If no priority is specified, Moonbeam defaults to prioritizing FPS.
:::
...
### Prioritizing Smooth Motion for a Fast-Paced Game

```bash
moonbeam --priority fps --max-fps 144 --min-fps 60 --max-resolution 1920x1080 --min-resolution 1280x720 stream MyHost "FastPacedGame"
```

This configuration prioritizes maintaining a high frame rate (between 60 and 144 FPS) and will lower the resolution if necessary to maintain smooth gameplay.

### Prioritizing Visual Quality for a Scenic Game

```bash
moonbeam --priority resolution --max-resolution 3840x2160 --min-resolution 1920x1080 --max-fps 60 --min-fps 30 stream MyHost "ScenicGame"
```

This setup prioritizes maintaining high resolution (between 1080p and 4K) and will lower the frame rate if necessary to preserve visual detail.
