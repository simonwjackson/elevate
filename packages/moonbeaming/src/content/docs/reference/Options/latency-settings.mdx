...
:::note
If you don't specify a `--max-latency` value, Moonbeam will not enforce a latency limit but will still attempt to minimize latency within the constraints of your other settings.
:::

## How Moonbeam Responds to Current Latency

1. Moonbeam measures the current network latency to your host using a simple `ping` test.
2. If the measured latency exceeds your set max latency, Moonbeam will warn you and may abort the stream.
3. The measured latency is used to optimize other streaming parameters, such as FPS.

## Best Practices

1. **Network Quality**: Your network quality significantly impacts latency. Consider these factors:
   - Physical distance to the host
   - Network congestion
   - Wi-Fi vs. Ethernet connection

2. **Game-Specific Tuning**: Adjust based on the type of game:
...
3. **Balancing Quality and Responsiveness**: Lower latency often requires sacrificing some video quality. Find the right balance for your needs.
...
<Badge text="High Latency" variant="caution"/> In some cases, you might encounter network conditions that result in unavoidably high latency (100ms or more). In these situations, some games are more forgiving and can still provide an enjoyable experience despite the higher latency. These games typically don't require split-second reactions or precise timing, allowing you to play comfortably even with noticeable input lag.
