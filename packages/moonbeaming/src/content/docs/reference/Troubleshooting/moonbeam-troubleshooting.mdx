...
1. <Badge text="QUIET" variant="danger" /> Only critical errors and fatal messages are displayed. 
    ...
    Use in production environments where minimal output is desired, or when integrating Moonbeam into automated systems where only critical issues need attention.

2. <Badge text="SIMPLE" variant="note" /> Basic operational information is logged (default).

    For everyday use in production, providing essential information without overwhelming the logs.
...
:::note
<Badge text="ERROR" variant="danger" /> and <Badge text="FATAL" variant="danger" /> messages are always displayed to ensure that users are aware of critical issues, even when lower verbosity levels are selected. This helps maintain system reliability and facilitates quick problem resolution.
:::
